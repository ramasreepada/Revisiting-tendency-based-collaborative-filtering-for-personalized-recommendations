    ...
    for(j in 1:ncol(Test_Matrix))
    {
      ...
      else
      {
        ...
        else
        {
          ...
          mean_ratings_of_these_users = as.matrix(Mean_rating_User[user_indexes_who_rated_current_item])
          
          all_ratings_of_users_who_Rated_current_item = Train_Matrix[(which(Train_Matrix[,j] > 0)),j]
          
          Tendancy_Item = sum(all_ratings_of_users_who_Rated_current_item - mean_ratings_of_these_users )/as.numeric(nrow(user_indexes_who_rated_current_item))
...
write.csv(Recomm,"Tendency Hybrid Results Clarity.csv",row.names = F,col.names = F)
